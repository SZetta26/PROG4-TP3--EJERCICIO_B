...
import { Stethoscope, Plus, Edit, Trash, Loader2, AlertTriangle, User, X } from 'lucide-react';
...
const ListaMedicos = () => {
    ...
    const fetchMedicos = async () => {
        ...
        setError(null);
        try {
            const response = await fetchAuth(`${BASE_URL}/api/medicos`);
            
            if (!response.ok) {
                const errorData = await response.json();
                throw new Error(errorData.message || `Error al cargar médicos: ${response.status}`);
            }
            ...
            console.error("Fetch Médicos Error:", err);
    ...
    const handleOpenForm = (medico = initialFormState) => {
        setFormData(medico);
        setIsEditing(!!medico.id);
        setShowForm(true);
        setError(null);
    };
    ...
    const handleSubmit = async (e) => {
        ...
        const url = isEditing ? `${BASE_URL}/api/medicos/${formData.id}` : `${BASE_URL}/api/medicos`;
        ...
        try {
            ...
            if (!response.ok) {
                const errorData = await response.json();
                throw new Error(errorData.message || `Error al ${isEditing ? 'actualizar' : 'crear'} médico.`);
            }
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-75 z-50 flex justify-center items-center p-4">
            <div className="bg-white rounded-xl shadow-3xl w-full max-w-sm p-6 text-center">
                <div className="flex justify-end">
                    <button onClick={() => setConfirmModal({ visible: false, medicoId: null, medicoNombre: '' })} className="text-gray-400 hover:text-gray-600">
                        <X className="w-5 h-5" />
                    </button>
                </div>
                ...
                <p className="text-gray-600 mb-6">
                    ¿Estás seguro de que deseas eliminar al médico **{confirmModal.medicoNombre}**? Esta acción no se puede deshacer.
                </p>
                <div className="flex justify-center space-x-4">
                    <button
                        onClick={() => setConfirmModal({ visible: false, medicoId: null, medicoNombre: '' })}
            ...
            <div className="container mx-auto p-4 sm:p-6 lg:p-8">
                <div className="flex items-center justify-between mb-8 border-b pb-4">
                    ...
                    <button
                        ...
                        className="flex items-center px-6 py-2 bg-red-600 text-white font-semibold rounded-lg shadow-lg hover:bg-red-700 transition duration-150 transform hover:scale-105"
                    >
                    ...
                    <div className="fixed inset-0 bg-gray-600 bg-opacity-75 z-50 flex justify-center items-center p-4">
                        <div className="bg-white rounded-xl shadow-3xl w-full max-w-lg p-6">
                            <h2 className="text-2xl font-bold text-gray-800 mb-4 border-b pb-2">
                                {isEditing ? 'Editar Médico' : 'Crear Médico'}
                            </h2>
                            <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                <div className="flex justify-end space-x-4 pt-4">
                                    ...
                                    >
                                        {loading ? <Loader2 className="w-5 h-5 animate-spin mx-auto" /> : (isEditing ? 'Guardar Cambios' : 'Crear Médico')}
                                    </button>
                                </div>
                            </form>
                        </div>
                ...
                {loading && !showForm && (
                    <div className="flex justify-center items-center p-6 bg-red-50 rounded-lg shadow-inner">
                        <Loader2 className="w-6 h-6 mr-3 text-red-500 animate-spin" />
                        <p className="text-red-700 font-medium">Cargando médicos...</p>
                    </div>
                )}
                    ...
                    <div className="bg-white rounded-xl shadow-2xl overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {medicos.map((medico) => (
                                    <tr key={medico.id} className="hover:bg-gray-50 transition duration-150">
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                            <User className="w-4 h-4 mr-2 inline text-red-500" />
                                            {`${medico.nombre} ${medico.apellido}`}
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{medico.matricula_profesional}</td> 
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-red-600">{medico.especialidad}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <button 
                                                onClick={() => handleOpenForm(medico)}
                ...
                {!loading && !error && medicos.length === 0 && (
                    <div className="text-center p-10 bg-white rounded-xl shadow-lg">
                        <p className="text-xl text-gray-500">No hay médicos registrados en el sistema.</p>
                        <p className="text-gray-400 mt-2">Haz clic en "Nuevo Médico" para comenzar.</p>
                    </div>
                )}
            </div>
        </AuthPage>
